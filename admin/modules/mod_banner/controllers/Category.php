...
class Category extends CI_Controller{
    ...
    function getContent(){
        ...
        if($list){
            foreach($list as $row){
                ...
                $data->action .= ($this->check->check('edit'))?icon_active("'banner_category'","'category_id'",$row->category_id,$row->bl_active):"";
                $data->action .= '</span>';
    ...
    function edit($id,$page = 0){
        ...
        $data['item'] = $this->category->get_category_item($id);
        $data['category'] = $this->category->getCategory();
