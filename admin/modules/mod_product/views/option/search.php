<form name="frmsearch" id="frmsearch" action="<?php echo base_url().$this->_module_name;?>/member/search/" method="post">
	...
	<p style="text-align:center"> Họ Tên : <input type="text" id="full_name" name="full_name" class="w200" value="<?php echo $full_name?>"> &nbsp; &nbsp;  Điện thoại : <input type="text" id="tel" name="tel" class="w200" value="<?php echo $tel?>">&nbsp; &nbsp; Từ Ngày : <input type="text" id="tungay" name="tungay" class="w200" value="<?php echo $tungay?>"> &nbsp;&nbsp;&nbsp;&nbsp;Đến Ngày: <input type="text" id="denngay" name="denngay" class="w200" value="<?php echo $denngay?>"> &nbsp;&nbsp;&nbsp;<input type="submit" class="submit"  name="btn_submit" value=" Lọc dữ liệu " style="height:36px"></p>
 </form>
<?
...
<table class="admindata">
   ...
   <tr class="row<?php echo $k?>">
       ...
       <td><?php echo $rs->cmnd;?></td>
        ...
        <td align="center">
        <a href="<?php echo base_url()."mod_game/member/game_score/".$rs->id?>" title="Xem các lượt chơi" class="cboxElement"><img src="<?php echo base_url()?>templates/images/lage.png" width=""/></a>
            <?php echo ($this->acl->check('edit'))?icon_edit($this->_module_name.'/member_game/edit/'.$rs->id):'';?>
            <span id="publish<?php echo $rs->id?>">
            <?php echo ($this->acl->check('edit'))?icon_active("'tb_member_game'","'id'",$rs->id,$rs->bl_active):'';?>
            </span>
            <?php echo ($this->acl->check('del'))?icon_del($this->_module_name.'/member_game/del/'.$rs->id.'/'.(int)$current_page):'';?>
        </td>
   </tr>
   <?
        $k=1-$k;
